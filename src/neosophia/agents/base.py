...
from neosophia.agents.utils import Resource, Tool, Variable
...
class Agent:
    ...
    def __init__(
        ...
        self.tools['extract_answer'] = Tool(
            name='extract_answer',
            function_str=None,
            description='Function to extract an answer given a question and data',
        ...
        self.tools['system_exit'] = Tool(
            name='system_exit',
            function_str=None,
            description='Function to exit the program',
            call=sys.exit
        )
    ...
    def chat(self):
        ...
        def get_command(prompt):
            print('\nAsk a question')
            user_input = ''
            user_input = 'How many customers own cats?'
            while user_input == '':
                user_input = input('> ')
            if user_input == 'exit':
                sys.exit(1)
        ...
        while True:
            ...
            for name, resource in self.resources.items():
                prompt.add_resource(name, resource)
            ...
            while True:
                ...
                if function == self.function_calls['extract_answer']:
                    ...
                    self.variables[return_name] = Variable(
                        name=return_name,
                        value=res,
                        display_name=None,
                        description=None)

                    prompt.add_variable(return_name, res)
        ...
        for key, value in parsed_data.items():
            if key.startswith(param_prefix):
                ...
                if param_value in self.variables:
                    param_value = self.variables[param_value]
